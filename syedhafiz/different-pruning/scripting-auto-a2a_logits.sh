...
ATTACK_NAME=(AutoProjectedGradientDescent Wasserstein CarliniWagner FastGradientMethod Deepfool UniversalPerturbation BasicIterativeMethod ElasticNet Wasserstein AdversarialPatch ShadowAttack NewtonFool TargetedUniversalPerturbation CarliniWagner)
...
do
    ...
    if [ $train_flag -eq 1 ]; then
        ...
        python3 smh-train-classifier-a2a-logits.py $N_TRAINING_SAMPLES $N_BATCH_SIZE $N_EPOCHS $CLASSIFIER_FILE_PREFIX ${MODEL_NAME[$MODEL_INDEX]} >> $PRINT_OUTPUT_FILE
        deactivate
    fi 
    ...
    do
        ...
        if [ $attack_flag -eq 1 ]; then
            ...
            python3 smh-attack-and-adv-examples-logits.py $CLASSIFIER_FILE_PREFIX ${DATASET[$DATASET_INDEX]} ${MODEL_NAME[$MODEL_INDEX]} ${ATTACK_NAME[$ATTACK_INDEX]} $N_TESTING_SAMPLES $N_BATCH_SIZE >> $PRINT_OUTPUT_FILE
            deactivate
        fi 
        ...
        echo $json_file
        python3 smh-nncf-results-logits.py ${DATASET[$DATASET_INDEX]} ${MODEL_NAME[$MODEL_INDEX]} ${ATTACK_NAME[$ATTACK_INDEX]} $N_ADV_SAMPLES $CLASSIFIER_FILE_PREFIX $json_file $N_BATCH_SIZE >> $PRINT_OUTPUT_FILE
        python3 smh-nncf-a2a-results-logits.py ${DATASET[$DATASET_INDEX]} ${MODEL_NAME[$MODEL_INDEX]} ${ATTACK_NAME[$ATTACK_INDEX]} $N_ADV_SAMPLES $CLASSIFIER_FILE_PREFIX $json_file $N_BATCH_SIZE >> $PRINT_OUTPUT_FILE
        deactivate
    done
done
echo "Done!"
