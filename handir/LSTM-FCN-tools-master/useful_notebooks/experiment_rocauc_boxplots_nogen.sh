...
# Loop over everything
    for m in 1 2 3; do
        ...
        python preprocessor.py -d unedited_data/${array[k]}.csv -p ${array[k]} -c 0.2  # largely have to go with 0.2
        python lda_nogen.py -d ${array2[k]} -p ${array[k]}
        mv weights/${array[k]}_LDA_micro_macro.csv ~/results/${array[k]}/rep_ROC

        for i in 0 1 2 3; do
            python trainer.py -d ${array2[k]} -m $i -r 1 -p ${array[k]} -e $epochs -b 8 -l ${array3[i]}  # -b 8
            sleep $seconds
