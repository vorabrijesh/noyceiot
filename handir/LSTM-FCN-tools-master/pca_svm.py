...
def pca_svm(datasetindex, prefix, gen):
    
    """
    Training and running PCA-SVM
    ...
    clf = SVC()
    y_score = clf.fit(X_t_train, y_train).decision_function(X_t_test)
    ...
    plt.savefig('figures/{prefix}_pcasvm_roc_curves.png'.format(prefix=prefix), bbox_inches='tight')
    ...
    micro_macro.to_csv('weights/{prefix}_{model_num}_micro_macro.csv'.format(model_num='PCASVM', prefix=prefix))



    # Classifcation_report
    ## print(classification_report(y_true, y_pred, target_names=target_names))
    print(classification_report(np.argmax(y_test, axis=1), clf.predict(X_t_test), 
            target_names=[l for l in read_dictionary.values()]),
    file=open('weights/{prefix}_{model_num}_report.txt'.format(model_num='PCASVM', prefix=prefix), "w"))
...
if __name__ == "__main__":
    ...
    pca_svm(args.datasetindex, args.prefix, args.gen)
