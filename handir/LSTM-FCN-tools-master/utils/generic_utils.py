...
from utils.constants import TRAIN_FILES, TEST_FILES, MAX_SEQUENCE_LENGTH_LIST, NB_CLASSES_LIST, X_AXIS_LIST, Y_AXIS_LIST
...
def plot_dataset(dataset_id, seed=None, limit=None, cutoff=None,
    ...
    axs[0][0].set_xlabel(X_AXIS_LIST[dataset_id])  # Potential
    axs[0][0].set_ylabel(Y_AXIS_LIST[dataset_id])    # Current
    train_df.plot(subplots=False,
                  legend='best',
                  ax=axs[0][0],)

    axs[0][1].set_title('Test dataset', size=16)
    axs[0][1].set_xlabel(X_AXIS_LIST[dataset_id]) # timestep
    axs[0][1].set_ylabel(Y_AXIS_LIST[dataset_id])    # value
    ...
    if plot_data is not None and X_train_attention is not None:
        ...
        axs[1][0].set_xlabel(X_AXIS_LIST[dataset_id])  # timestep
        axs[1][0].set_ylabel(Y_AXIS_LIST[dataset_id])     # value
    ...
    if plot_data is not None and X_test_attention is not None:
        ...
        axs[1][1].set_xlabel(X_AXIS_LIST[dataset_id])  # timestep
        axs[1][1].set_ylabel(Y_AXIS_LIST[dataset_id])     # value
