...
def train_model(model:Model, dataset_id, dataset_prefix, epochs=50, batch_size=128, model_num=None, val_subset=None,
    ...
    model_checkpoint = ModelCheckpoint("./weights/{dataset_prefix}_{model_num}_weights.h5".format(dataset_prefix=dataset_prefix,
                                       model_num=model_num), verbose=1,
...
def evaluate_model(model:Model, dataset_id, dataset_prefix, batch_size=128, model_num=None, test_data_subset=None,
    ...
    model.load_weights("./weights/{dataset_prefix}_{model_num}_weights.h5".format(dataset_prefix=dataset_prefix, model_num=model_num))
...
def visualize_context_vector(model:Model, dataset_id, dataset_prefix, cutoff=None, limit=None, model_num=None,
...
def visualize_cam(model:Model, dataset_id, dataset_prefix, class_id,
                  cutoff=None, normalize_timeseries=False, model_num=None):
    X_train, y_train, _, _, is_timeseries = load_dataset_at(dataset_id,
                                                           normalize_timeseries=normalize_timeseries)
    _, sequence_length = calculate_dataset_metrics(X_train)
    ...
    conv_cam /= conv_cam.max()  ######################################## doesn't work with model 1?
    ...
    plt.show()
    return sequence_df, conv_cam_df
